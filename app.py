...
# Si aún no encuentra, mostrar error con debug
if len(total) == 0:
    st.error("No se encontró la categoría 'Total National Health Expenditures'.")
    st.write("**Categorías disponibles:**")
    st.dataframe(pd.DataFrame({"Categoría": sorted(filtered["Expenditure_Type"].unique())}))
    st.stop()
else:
    st.success(f"Categoría encontrada: {total['Expenditure_Type'].iloc[0]} ({len(total)} registros)")
